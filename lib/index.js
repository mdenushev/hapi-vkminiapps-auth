  ...
  return {
    authenticate: async function (request, h) {
      const headers_schema = Joi.object({
        ...
        vk_ref: Joi.string().pattern(/^(featuring_(menu|discover|new)|other|catalog(_.*)?|group_menu|left_nav|quick_search|super_app)$/),
      ...
      if (typeof settings.validate === 'function') {
        const result = await settings.validate(ordered);
        if (result.isValid) {
